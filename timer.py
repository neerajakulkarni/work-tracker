...
def job():
    ...
    def totalcalc(actwin, timesp, listt):
        if sys.platform in ['Mac', 'darwin', 'os2', 'os2emx']:
            if actwin not in listt and actwin != 'loginwindow':
                listt[actwin] = int(timesp)
            elif actwin in listt and actwin != 'loginwindow':
                listt[actwin] = listt[actwin] + int(timesp)
        elif sys.platform in ['Windows', 'win32', 'cygwin']:
            if actwin not in listt and actwin != "" and actwin.startswith('http')==False:
                listt[actwin] = int(timesp)
            elif actwin in listt and actwin != "" and actwin.startswith('http')==False:
                listt[actwin] = listt[actwin] + int(timesp)
    ...
    while True:
        ...
        if str(datetime.datetime.now().time()).startswith('17:40:30'):
